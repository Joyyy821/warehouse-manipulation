                    ...
                    object_color_detector::DetectObjectSrv::Response &res)
{
    ...
    while(ros::ok()) {
        try
        {
            msg = ros::topic::waitForMessage<sensor_msgs::Image>(image_topic, ros::Duration(5.0));
        }
	...
	if (msg->header.stamp > t) break;
       	ros::Duration(0.05).sleep();
      
        if ((ros::Time::now() - t).toSec() > 5) {
            ROS_ERROR("Timeout waiting for image data");
            res.result = object_color_detector::DetectObjectSrvResponse::TIMEOUT;
            return false;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    // hsv_object[0].smin = 45;   // For red (pink) object, change the s detection range.
    // hsv_object[0].smax = 255;

    image_transport::Subscriber image_sub = it_.subscribe(image_topic, 1, &imageCb);
